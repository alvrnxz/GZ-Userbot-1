...
from gezet.database import gmutedb as Gmute
...
async def gban_user(client: Client, message: Message):
    ...
    if user_id in DEVS:
        return await ex.edit("**jangan sok keras dek 🗿**")
    if user_id:
        try:
    ...
    if (await gezet.gban_info(user.id)):
...
async def ungban_user(client: Client, message: Message):
    ...
    try:
        if not (await gezet.gban_info(user.id)):
        ...
        for good_boi in ung_chats:
            try:
                ...
                er += 1
        await gezet.ungban_user(user.id)
        msg = (
            r"**\\#UnGbanned_User//**"
            f"\n\n**First Name:** [{user.first_name}](tg://user?id={user.id})"
            f"\n**User ID:** `{user.id}`"
...
if ok:
 ...
 async def globals_check(client: Client, message: Message):
    ...
    if not user_id:
        return
    if (await gezet.gban_info(user_id)):
        try:
